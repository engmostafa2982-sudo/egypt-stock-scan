"""
daily_scan.py — Full automated daily scan: ALL EGX (Egyptian Exchange) stocks
30-point technical scoring system. Runs at 8:30 AM Saudi time (5:30 AM UTC).
"""

import os, sys, json, csv, glob, time, datetime, math
...
def run_all_chunks():
    log("Starting EGX scoring...")
    import score_engine as se

    universe_path = os.path.join(BASE, "universe.csv")
    if not os.path.exists(universe_path):
        log("  universe.csv not found — collecting...")
        import universe_collect
        universe_collect.collect_universe()
    ...
    log(f"  {total} EGX stocks → 8 chunks of ~{chunk_size} each")
...
def build_report():
    ...
    for i, r in enumerate(scored, 1):
        ...
        avgvol  = f"{vol_r/1e6:.1f}M" if vol_r>=1e6 else f"{vol_r/1e3:.0f}K" if vol_r>=1e3 else "N/A"
        sc      = score_color(final)
        bg      = '#1a2332' if i%2==0 else '#151e2d'

        rows_parts.append(
            f'<tr style="background:{bg}" onmouseover="this.style.background=\'#1e3a5f\'" onmouseout="this.style.background=\'{bg}\'">'
            ...
            f'<td style="text-align:right;color:#80cbc4">{price:.2f} EGP</td>'
...
<head>
<meta charset="UTF-8">
<title>EGX Full Market Scan | {DATE_STR}</title>
<style>
...
<body>
<div class="header">
  <h1>&#127466;&#127468; EGX FULL MARKET SCAN &mdash; ALL STOCKS</h1>
  <h2>30-Point Technical Scoring System v2 &nbsp;|&nbsp; {DATE_STR} &nbsp;|&nbsp; Powered by EODHD</h2>
  <a class="run-btn" href="https://github.com/engmostafa2982-sudo/egypt-stock-scan/actions/workflows/daily_scan.yml" target="_blank">&#9654; Run New Scan Now</a>
</div>
<div class="stats">
...
<div class="footer">
  ...
  All {len(scored)} scored EGX stocks &nbsp;|&nbsp; {DATE_STR}
</div>
<script>
...
function sortTable(col) {{
  ...
  table.setAttribute('data-sort-dir', asc ? 'desc' : 'asc');
    ...
    html_path = os.path.join(BASE, f"EGX_Stock_Scan_{TODAY}.html")
    with open(html_path, 'w', encoding='utf-8') as f:
        f.write(html)
    size_kb = os.path.getsize(html_path) // 1024
    log(f"  HTML saved: EGX_Stock_Scan_{TODAY}.html ({size_kb} KB)")
    return html_path


if __name__ == '__main__':
    log("=" * 60)
    log(f"EGX DAILY SCAN STARTED — {DATE_STR}")
    log("=" * 60)
    t0 = time.time()
    try:
        clear_stale()
